...
static void comboviewshift(const Arg *arg);
...
#if DEBUG == 1
	fprintf(logfile, "comboview newtags %d \n", newtags);
	fprintf(logfile, "comboview newtags %d \n", newtags);
#endif
	focus(NULL);
	arrange(selmon);
...
comboviewshift(const Arg *arg)
{
	...
	if (arg->i == +1)
	{
		newtags = (selmon->tagset[selmon->seltags] << 1) & TAGMASK;
#if DEBUG == 1
		fprintf(logfile, "comboviewshift newtags = %d \n", newtags);
#endif
		if (newtags == 0)
		{
			newtags = 1;
#if DEBUG == 1
			fprintf(logfile, "NEWTAGS = 0\n");
			fprintf(logfile, "newtags = %d \n", newtags);
#endif
		}
	}
	else if (arg->i == -1)
	...
	{
		newtags = selmon->tagset[selmon->seltags] & TAGMASK;
	}
	Arg newarg = {.ui = newtags};
	comboview(&newarg);
}
...
void setup(void)
{
	...
	if (logfile == NULL)
	{
		die("cannot open logfile %s", DEBUGPATH);
	}
	setvbuf(logfile, (char *)NULL, _IOLBF, 0);
	setvbuf(logfile, (char *)NULL, _IOLBF, 0);
	fputs("\n#####BEGGINING OF NEW SESSION######\n", logfile);
